...
export class Marcas implements OnInit {
  ...
  readonly filtrados = computed(() => {
    ...
    return this.inventario.marcas().filter((item) => {
      const matchQ =
        !q ||
  ...
  cargar(): void {
    ...
    this.inventario.listarMarcas().subscribe({
      next: () => this.loading.set(false),
      error: (err) => {
        this.loading.set(false);
        this.errorMessage.set(apiErrorMessage(err, 'No se pudieron cargar las marcas.'));
      },
    });
  }
  ...
  guardar(): void {
    ...
    const request = this.editingId()
      ? this.inventario.actualizarMarca(this.editingId()!, payload)
      : this.inventario.crearMarca(payload);
    request.subscribe({
      next: () => {
  ...
  cambiarEstado(item: Marca): void {
    this.saving.set(true);
    this.errorMessage.set('');
    this.inventario.actualizarMarca(item.id, { estado: !item.estado }).subscribe({
